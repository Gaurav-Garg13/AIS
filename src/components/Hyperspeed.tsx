...
// The big Hyperspeed implementation is kept as close as possible to the source,
// but slightly adapted to work nicely in React/TypeScript.

const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: { effectOptions?: Partial<EffectOptions> }) => {
  ...
  useEffect(() => {
    try {
      ...
      options.distortion =
        distortions[options.distortion as keyof typeof distortions] ?? distortions.turbulentDistortion;
      ...
      // Fail silently so the About page still works even if WebGL fails.
...
// ----------------- Original Hyperspeed implementation (trimmed comments) -----------------
...
class App {
  ...
  loadAssets() {
    ...
    return new Promise<void>((resolve) => {
      ...
      assets.smaa = {};
      searchImage.addEventListener('load', function handleLoad() {
        assets.smaa.search = this;
        manager.itemEnd('smaa-search');
      });

      areaImage.addEventListener('load', function handleLoad() {
...
class CarLights {
  ...
  init() {
    ...
    for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
      ...
      aColor.push((color as THREE.Color).r, (color as THREE.Color).g, (color as THREE.Color).b);
      aColor.push((color as THREE.Color).r, (color as THREE.Color).g, (color as THREE.Color).b);
    }
